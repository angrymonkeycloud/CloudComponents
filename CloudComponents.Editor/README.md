...
[![NuGet](https://img.shields.io/nuget/v/AngryMonkey.CloudComponents.Editor?logo=nuget)](https://www.nuget.org/packages/AngryMonkey.CloudComponents.Editor)
[![NuGet Downloads](https://img.shields.io/nuget/dt/AngryMonkey.CloudComponents.Editor?logo=nuget)](https://www.nuget.org/packages/AngryMonkey.CloudComponents.Editor)
...
> `CloudEditor` is the single entry point. Bind `@bind-Value` to an HTML string and configure everything else through parameters.
...
- [Component API](#component-api)
  - [`CloudEditor` parameters](#cloudeditor-parameters)
  - [Public methods](#public-methods)
  - [`CloudEditorToolbarOptions`](#cloudeditortoolbaroptions)
...
```razor
<CloudEditor @bind-Value="_html" Placeholder="Start writing…" />
...
| `Toolbar` | `CloudEditorToolbarOptions` | all `true` | Feature toggles per tool group. |
...
| `ImageUploadHandler` | `Func<CloudEditorFileUpload, Task<string?>>?` | `null` | Persists an uploaded image and returns its URL. Falls back to inline data URLs when `null`. |
...
<CloudEditor @bind-Value="_html"
             Toolbar="new CloudEditorToolbarOptions { Video = false, Fullscreen = false }" />
```
...
```razor
<CloudEditor @bind-Value="_html" FirstHeadingLevel="2" HeadingCount="3" />
```
...
The `</>` toolbar button switches to an HTML source editor. Source is validated live by `CloudEditorHtmlValidator`:
...
foreach (CloudEditorHtmlValidationMessage warning in result.Warnings)
...
@code {
    private async Task<string?> UploadAsync(CloudEditorFileUpload file)
    {
...
`CloudEditorFileUpload` exposes `FileName`, `ContentType`, `Size` (bytes), and `OpenReadStream()`.
...
- **Component doesn't render / JS error on load** — ensure the project references `AngryMonkey.CloudComponents.Editor` and the `@using` directives (or `_Imports.razor` entries) are in place.

## Changelog

### 4.1.3

- Initial release as a standalone NuGet package (`AngryMonkey.CloudComponents.Editor`).
...
- HTML code view with `CloudEditorHtmlValidator` — errors block Apply, warnings are informational.
- Paste sanitization with allowlist.
- Image upload handler integration (`CloudEditorFileUpload`).
...
| `Toolbar` | `CloudEditorToolbarOptions` | all on | Feature toggles per tool group. |
| `SanitizePaste` | `bool` | `true` | Sanitizes HTML pasted from the clipboard. |
| `DebounceMilliseconds` | `int` | `250` | Delay before `Value` updates while typing. |
| `ImageUploadHandler` | `Func<CloudEditorFileUpload, Task<string?>>?` | `null` | Persists uploads and returns a URL. Falls back to inline data URLs. |
...
```razor
<CloudEditor @bind-Value="_html" Toolbar="new() { Video = false, Fullscreen = false }" />
```
...
The code view button (`</>`) switches to an HTML source editor. Source is validated live by `CloudEditorHtmlValidator`:
...
```razor
<CloudEditor @bind-Value="_html" ImageUploadHandler="UploadAsync" />
