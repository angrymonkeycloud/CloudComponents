...
/// <summary>
/// Options for the built-in <c>CloudDataGridHeader</c> rendered above a <c>CloudDataGrid</c>.
/// When this is passed to <see cref="Components.CloudDataGrid.Header"/> the header is shown;
/// leave it null to hide the header entirely.
/// </summary>
public class CloudDataGridHeaderOptions
{
    ...
    /// <list type="bullet">
    ///   <item>Actions with <see cref="CloudDataGridAction.ShowOnHeader"/> appear on the right.</item>
    ///   <item>Actions with <see cref="CloudDataGridAction.ShowOnBulkHeader"/> appear on the left
    ///     (next to the label) only while rows are selected.</item>
    /// </list>
    /// </summary>
    public List<CloudDataGridAction> Actions { get; set; } = [];

    /// <summary>
    /// Raised when any <see cref="CloudDataGridAction"/> of type <see cref="CloudDataGridActionType.Button"/>
    /// is clicked. The event args carry the action and any relevant record ids.
    /// </summary>
    public EventCallback<CloudDataGridActionEventArgs> OnActionClicked { get; set; }
    ...
    /// <see cref="CloudDataGridAction"/> model.
    /// </summary>
