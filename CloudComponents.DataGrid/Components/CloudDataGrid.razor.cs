...
/// Composes <see cref="CloudDataGridHeader"/> (optional), <see cref="CloudDataGridBody"/>,
/// and <see cref="CloudDataGridFooter"/> internally.
///
...
/// </summary>
public partial class CloudDataGrid : IAsyncDisposable
{
    #region Parameters

    /// <summary>When set, renders a <see cref="CloudDataGridHeader"/> above the table.</summary>
    [Parameter] public CloudDataGridHeaderOptions? Header { get; set; }
    ...
    /// or search change. Return <see cref="CloudDataGridDataResult.ErrorMessage"/> to surface
    /// a soft error instead of rows.
    /// </summary>
    [Parameter] public required Func<CloudDataGridDataRequest, Task<CloudDataGridDataResult?>> DataProvider { get; set; }
    ...
    /// Once content grows past this height the grid scrolls internally, matching <see cref="CloudDataGridHeightMode.FullHeight"/>.
    /// Ignored for the other height modes.
    /// </summary>
    [Parameter] public double? MaxHeight { get; set; }

    /// <summary>How the grid navigates between pages. Defaults to <see cref="CloudDataGridPagingMode.Pages"/>.</summary>
    [Parameter] public CloudDataGridPagingMode PagingMode { get; set; } = CloudDataGridPagingMode.Pages;

    /// <summary>Text of the load-more button when <see cref="PagingMode"/> is <see cref="CloudDataGridPagingMode.LoadMore"/>.</summary>
    ...
    private CloudDataGridDataResult? _data;
    private bool _loading;
    private bool _searching;
    private string? _search;
    private CloudDataGridSort? _sort;
    ...
    private async Task ExecuteAsync(int page, bool isAppend, bool isSearch = false)
    {
        ...
        CloudDataGridDataRequest request = new()
        {
            Page = page,
            PageSize = PageSize,
            Search = _search,
        ...
        CloudDataGridDataResult? result = null;
        ...
        {
            result = new CloudDataGridDataResult { ErrorMessage = ex.Message };
        }
        finally
        {
            ...
            {
                _data = result ?? new CloudDataGridDataResult { ErrorMessage = "Failed to load data." };
            }
    ...
    private Task OnPageChanged(CloudDataGridPaginationType type)
    {
        if (_data == null) return Task.CompletedTask;

        if (type == CloudDataGridPaginationType.LeftArrow)
        {
            int prev = _data.Page - 1;
            return prev < 1 ? Task.CompletedTask : ExecuteAsync(prev, isAppend: false);
        }

        bool append = PagingMode is CloudDataGridPagingMode.InfiniteScroll or CloudDataGridPagingMode.LoadMore;
    ...
    private async Task ExportAllAsync()
    {
        ...
        CloudDataGridDataResult? allData = null;

        try
        {
            allData = await DataProvider(new CloudDataGridDataRequest
            {
                Page = 1,
    ...
    {
        if (_closeExportPanel != null) await _closeExportPanel();
        IEnumerable<CloudDataGridRow> rows = _data?.Rows.Where(r => _selectedRecords.Contains(r.Id)) ?? [];
        await DownloadCsvAsync(rows, "selection");
    }

    private async Task DownloadCsvAsync(IEnumerable<CloudDataGridRow> rows, string scope)
    {
    ...
    private string BuildCsv(IEnumerable<CloudDataGridRow> rows)
    {
    ...
    /// <summary>
    /// Supplied to the built-in Export <see cref="CloudDataGridAction"/> as its ChildContent.
    /// Defined as a property so it can reference instance methods.
    /// Assigned in CloudDataGrid.razor @code block.
    /// </summary>
    private RenderFragment? _exportContent;

    /// <summary>
    /// Callback that closes the export focus panel. Set as the Export action's
    /// <see cref="CloudDataGridAction.OnDeactivated"/> so the header can trigger it,
    ...
    /// </summary>
    private CloudDataGridHeaderOptions? ActiveHeader
    {
        get
        {
            ...
            List<CloudDataGridAction> actions = [.. Header.Actions];
            ...
            if (Header.AllowExport && _exportContent != null)
            {
                ...
                {
                    actions.Add(new CloudDataGridAction
                    {
                        Key = KeyExport,
                        Text = "Export",
                        Tooltip = "Export",
            ...
            return new CloudDataGridHeaderOptions
            {
                Label = Header.Label,
                Actions = actions,
    ...
    private List<CloudDataGridAction> AllRowActions
    {
        get
        {
            IEnumerable<CloudDataGridAction> fromHeader = Header?.Actions.Where(a => a.ShowOnRow) ?? [];
            IEnumerable<CloudDataGridAction> explicit_ = RowActions ?? [];
    ...
    private int FooterRangeStart => _data == null ? 0
        : PagingMode == CloudDataGridPagingMode.Pages ? (_data.PageSize * (_data.Page - 1)) + 1
        : 1;

    private int FooterRangeEnd => _data == null ? 0
        : PagingMode == CloudDataGridPagingMode.Pages ? (_data.PageSize * (_data.Page - 1)) + _data.Rows.Count
        : _data.Rows.Count;
    ...
    private string RootClasses
    {
        get
        {
            ...
            if (HeightMode == CloudDataGridHeightMode.FullHeight) classes.Add("_full-height");
