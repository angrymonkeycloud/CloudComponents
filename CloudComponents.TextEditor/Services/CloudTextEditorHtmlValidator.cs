...
namespace AngryMonkey.CloudComponents.TextEditor.Services;

/// <summary>
/// Lightweight HTML fragment validator used by the <c>CloudTextEditor</c> code view.
...
/// </summary>
public static class CloudTextEditorHtmlValidator
{
    ...
    public static CloudTextEditorHtmlValidationResult Validate(string? html)
    {
        List<CloudTextEditorHtmlValidationMessage> messages = [];
        ...
        while (index < html.Length)
        {
            ...
            if (Matches(html, index, "<!--"))
            {
                ...
                if (end < 0)
                {
                    AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, index, "Comment is never closed. Expected '-->'.");
                    break;
                }
            ...
            if (Matches(html, index, "<!"))
            {
                ...
                if (end < 0)
                {
                    AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, index, "Markup declaration is never closed. Expected '>'.");
                    break;
                }
            ...
            AddMessage(messages, CloudTextEditorValidationSeverity.Warning, lineStarts, index, "Unescaped '<' treated as text. Use '&lt;' instead.");
            index++;
        }

        foreach (OpenTag open in stack.Where(tag => !OptionalEndElements.Contains(tag.Name)))
            messages.Add(new(CloudTextEditorValidationSeverity.Error, open.Line, open.Column, $"Element <{open.Name}> is never closed."));

        return new() { Messages = [.. messages.OrderBy(m => m.Line).ThenBy(m => m.Column)] };
    }

    private static int ParseOpenTag(string html, int start, int[] lineStarts, List<OpenTag> stack, List<CloudTextEditorHtmlValidationMessage> messages)
    {
        int index = start + 1;
        ...
        while (true)
        {
            ...
            {
                AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, start, $"Tag <{name}> is never closed. Expected '>'.");
                return index;
            }
            ...
            {
                AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, index, $"Unexpected character '{html[index]}' inside tag <{name}>.");
                index++;
                continue;
            }

            if (!seenAttributes.Add(attributeName))
                AddMessage(messages, CloudTextEditorValidationSeverity.Warning, lineStarts, attributeStart, $"Duplicate attribute '{attributeName}' on <{name}>.");

            if (attributeName.StartsWith("on", StringComparison.OrdinalIgnoreCase) && attributeName.Length > 2)
                AddMessage(messages, CloudTextEditorValidationSeverity.Warning, lineStarts, attributeStart, $"Inline event handler '{attributeName}' is not allowed and will be removed.");
            ...
            if (index < html.Length && html[index] == '=')
            {
                ...
                if (index >= html.Length)
                {
                    AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, attributeStart, $"Attribute '{attributeName}' has no value.");
                    return index;
                }
                ...
                if (html[index] is '"' or '\'')
                {
                    ...
                    if (valueEnd < 0)
                    {
                        AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, index, $"Attribute '{attributeName}' value quote is never closed.");
                        return html.Length;
                    }
        ...
        if (DiscouragedElements.Contains(name))
            AddMessage(messages, CloudTextEditorValidationSeverity.Warning, lineStarts, start, $"<{name}> is discouraged inside editor content and may be removed.");
        ...
        if (RawTextElements.Contains(name))
        {
            ...
            if (close < 0)
            {
                AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, start, $"Element <{name}> is never closed.");
    ...
    private static int ParseCloseTag(string html, int start, int[] lineStarts, List<OpenTag> stack, List<CloudTextEditorHtmlValidationMessage> messages)
    {
        int index = start + 2;
        string name = ReadName(html, ref index);
        ...
        if (name.Length == 0)
        {
            AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, start, "Malformed closing tag.");
            return end < 0 ? html.Length : end + 1;
        }

        if (end < 0)
        {
            AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, start, $"Closing tag </{name}> is never closed. Expected '>'.");
            return html.Length;
        }
        ...
        {
            AddMessage(messages, CloudTextEditorValidationSeverity.Warning, lineStarts, start, $"Void element <{name}> should not have a closing tag.");
            return index;
        }
        ...
        if (matchIndex < 0)
        {
            AddMessage(messages, CloudTextEditorValidationSeverity.Error, lineStarts, start, $"Closing tag </{name}> has no matching opening tag.");
            return index;
        }
        ...
        for (int i = stack.Count - 1; i > matchIndex; i--)
        {
            ...
            if (!OptionalEndElements.Contains(unclosed.Name))
                messages.Add(new(CloudTextEditorValidationSeverity.Error, unclosed.Line, unclosed.Column, $"Element <{unclosed.Name}> is not closed before </{name}>."));
    ...
    private static void AddMessage(List<CloudTextEditorHtmlValidationMessage> messages, CloudTextEditorValidationSeverity severity, int[] lineStarts, int index, string message) =>
